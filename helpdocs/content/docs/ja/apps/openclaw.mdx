...
OpenClaw は完全にオープンソースであり、[OpenClaw の GitHub リポジトリ](https://github.com/openclaw/openclaw)でソースコードを閲覧したり、Issueを提出したり、貢献したりできます。このチュートリアルでは、OpenClaw のインストール、設定、および New API との連携の完全な手順を説明します。
...
<Callout type="info" title="準備情報">
  - Node.js 22 以降のバージョン
  - 利用可能な New API アドレス（通常 `/v1` で終わる）
  - 利用可能な New API API Key
  - ご自身でデプロイした New API を使用するか、サービスプロバイダーが合法的なアップストリーム認証とコンプライアンス義務を持つ New API サービスであることを確認してください。出所の不明な API アドレスやキーを本番環境に接続しないでください。
</Callout>
...
| `models.providers.newapi.baseUrl` | New API のアドレス。通常 `/v1` を含める必要があります。 |
| `models.providers.newapi.apiKey` | New API キー。`${NEWAPI_API_KEY}` を介して注入することをお勧めします。 |
| `models.providers.newapi.api` | New API のような OpenAI 互換ゲートウェイの場合、`openai-completions` を使用します。 |
| `models.providers.newapi.models` | ここにリストされているモデルIDは、New API が実際に公開しているモデル名と一致している必要があります。 |
